...
import type { BuiltInReporterName, ReporterName } from "../types/ReporterInitOptions.js";

const isCI = process.env.CI || process.env.TF_BUILD;

export type OptionCategory = "logger" | "pool" | "runner" | "run" | "server" | "filter" | "affected" | "cache" | "info";

export function getOptions(category: OptionCategory, config: ConfigOptions | undefined): Record<string, Option> {
  let options: Record<string, Option>;
  switch (category) {
    case "logger":
      if (!config) {
        throw new Error("config is required for logger options");
      }
      options = {
        reporter: getReporterOption(config),
        ...
        logLevel: new Option("--log-level <level>", "log level")
          .choices(["info", "warn", "error", "verbose", "silly"])
          .conflicts("verbose"),
  ...
  for (const [name, option] of Object.entries(options)) {
    ...
    const upperCaseSnakeKey = category.replace(/([A-Z])/g, "_$1").toUpperCase();
...
function getReporterOption(config: ConfigOptions): Option {
  const reporterChoices = Object.keys({
    json: true,
    azureDevops: true,
    npmLog: true,
    verboseFileLog: true,
    ...
    adoLog: true,
    old: true,
    // This verifies all supported reporters are listed
  } satisfies Record<Exclude<BuiltInReporterName, "default" | "profile">, true>);
  if (config.reporters) {
    reporterChoices.push(...Object.keys(config.reporters));
  }
  const lowerReporterChoices = Object.fromEntries(reporterChoices.map((v) => [v.toLowerCase(), v]));

  return new Option("--reporter <reporter...>", "reporter")
    .choices(reporterChoices)
    .argParser((value: string, previous: ReporterName[]): ReporterName[] => {
      const newValues = Array.isArray(value) ? value : value.split(",");
      const values = [...(previous || [])];

      for (const val of newValues) {
        const value = lowerReporterChoices[val.toLowerCase()] as ReporterName | undefined;
        if (!value) {
          throw new InvalidArgumentError(`Allowed values are: ${reporterChoices.join(", ")}`);
        }
